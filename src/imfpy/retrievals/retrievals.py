def dots(country, counterparts, start, end, freq="A", form="wide"):
    ...
    if isinstance(counterparts, list):
        assert (
            len(counterparts) > 1
        ), "counterparts must be a str or list of length 2 or more"
    ...
    def retrieve(counterpart):
        ...
        assert (
            r.status_code == 200
        ), "Error - HTTP Request unsuccessful. Please try again."
        ...
        assert len(exports) == len(
            imports
        ), "Error - data not available. Try a different time period or frequency."
    ...
    if isinstance(counterparts, list):
        ...
        for counterpart in counterparts:
            ...
            full_df = full_df.append(
                retrieved
            )  # if long-form data requested, stop here
        ...
        if form == "wide":  # otherwise, pivot to wide form data
            ...
            full_df.insert(
                1, "Country", country
            )  # if long-form data, insert country at position 1
