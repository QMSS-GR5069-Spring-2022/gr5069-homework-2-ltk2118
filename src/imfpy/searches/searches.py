...
def database_codes():
    ...
    if database_cache.empty:
        ...
        parsed_Name = pd.DataFrame(
            [
                database["Name"]
                for database in data_json["Structure"]["Dataflows"]["Dataflow"]
            ]
        )
        parsed_KeyFamilyRef = pd.DataFrame(
            [
                database["KeyFamilyRef"]
                for database in data_json["Structure"]["Dataflows"]["Dataflow"]
            ]
        )
...
def database_search(keyword, regex=False):
    ...
    if regex == False:
        keyword = keyword.lower()
        match = codes[
            codes["Description"].str.lower().str.contains(keyword, regex=False)
        ]
    else:
...
def database_info(database_id):
    ...
    assert (
        codes["Database ID"].str.fullmatch(database_id).any()
    ), "Invalid database. Please try again."
    ...
    annotations_json = data_json["Structure"]["KeyFamilies"]["KeyFamily"][
        "Annotations"
    ]["Annotation"]
    ...
    text_raw = [
        annotation["AnnotationText"]["#text"] for annotation in annotations_json
    ]
    ...
    text_clean = [
        re.sub(r"<.*?>|&([a-z0-9]+|#[0-9]{1,6}|#x[0-9a-f]{1,6});", "", text)
...
def database_dimensions(database_id):
    ...
    dimensions_temp = data_json["Structure"]["KeyFamilies"]["KeyFamily"]["Components"][
        "Dimension"
    ]

    # return neat dataframe of database dimensions, dropping redundant columns
    dimensions_temp = pd.DataFrame(dimensions_temp)[
        ["@conceptRef", "@conceptSchemeRef", "@codelist"]
    ]
    dimensions = dimensions_temp.rename(
        columns={
            "@conceptRef": "Concept",
...
def indicator_dimensions(indicator_id):
    ...
    indicator_dimensions = pd.DataFrame(
        {"Indicator ID": indicator_id, "Series ID": codes, "Description": descriptions}
    )
