...
def dotsplot(dots_dataframe, subset=["Exports", "Imports", "Trade Balance"]):
    ...
    assert isinstance(
        dots_dataframe, pd.DataFrame
    ), "dots_dataframe must be a DataFrame"
    assert isinstance(subset, list), "Subset must be a list"
    possible = {"Exports", "Imports", "Trade Balance", "Twoway Trade"}
    assert set(subset).intersection(possible) == set(
        subset
    ), "Subset has invalid inputs. Only 'Exports', 'Imports', 'Twoway Trade' and 'Trade Balance' are allowed."
    ...
    axes = grouped.plot(sharex=True, sharey=False, legend=True, linewidth=0.8)
